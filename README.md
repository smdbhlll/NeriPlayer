...
```text
NeriPlayer
├── 多源在线播放：网易云 / Bilibili / YouTube Music；可选 LX Music 自定义源
...
- 🔁 **网易云播放兜底**：
  ...
  可读的本地音频。
- 🔌 **LX Music 自定义源（用户自备）**：
  在“设置 > 播放源”可导入兼容 **LX Music 2.0** 的 `.js` 自定义源。支持同时保存多个源、
  单独启用或停用、调整尝试顺序与删除；网易云曲目会按列表顺序尝试已启用的源，并在播放页显示实际命中的 LX 源。
  “仅 LX 源”默认关闭，开启后会跳过原生播放地址、播放缓存及其他兜底，只使用已启用 LX 源解析网易云曲目，
  便于测试自定义源是否可用。自定义源是第三方可执行脚本，请只导入你信任且有权使用的来源。
- 🧯 **播放失败兜底**：
